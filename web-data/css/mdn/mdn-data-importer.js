...
const {
	propertyDescriptions: mdnPropertyDescriptions,
	fetchDocFromMDN,
} = require("./mdn-documentation");
...
async function addMDNProperties(vscProperties) {
	...
	for (const property of Object.keys(mdnProperties)) {
		allMDNProperties[property] = extractMDNProperties(
			undefined,
			mdnProperties[property],
			mdnCompatProperties[property],
		);
	}
	...
	for (const atRuleName of Object.keys(mdnAtRules)) {
		if (mdnAtRules[atRuleName].descriptors) {
			for (const atRulePropertyName of Object.keys(
				mdnAtRules[atRuleName].descriptors,
			)) {
				allMDNProperties[atRulePropertyName] = extractMDNProperties(
					atRuleName,
					mdnAtRules[atRuleName].descriptors[atRulePropertyName],
					mdnCompatAtRules[atRuleName]?.[atRulePropertyName],
				);
			}
		}
	}
